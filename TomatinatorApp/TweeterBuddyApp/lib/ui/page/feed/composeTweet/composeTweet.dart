...
class _ComposeTweetReplyPageState extends State<ComposeTweetPage> {
  ...
  File _image;
  TextEditingController _textEditingController; // declaration
  ...
  void initState() {
    ...
    scrollController = ScrollController();
    _textEditingController = TextEditingController(); // initialization
  ...
  /// If tweet is new tweet then `parentkey` and `childRetweetkey` should be null
  /// IF tweet is a comment then it should have `parentkey`
  /// IF tweet is a retweet then it should have `childRetweetkey`
  ...
  Future<void> postToTwitter() async {
    if (_textEditingController.text == null ||
        _textEditingController.text.isEmpty ||
        _textEditingController.text.length > 280) {
      return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        title: customTitleText('Compose Tweet'),
        onActionPressed: () async {
          kScreenloader.showLoader(context);
          String output = await Provider.of<ComposeTweetState>(context, listen: false)
          ...
          bool result = await showDialog(
            ...
            builder: (context) {
              return AlertDialog(
                title: Text('TweeterBuddy thinks... '),
                content: Text(output),
                actions: <Widget>[
                  /*
                  new TextButton(
                    onPressed: Provider.of<AuthState>(context, listen: false).isSignInWithTwitter
                        ? postToTwitter
                        : null,
                    child: Text('Post to Twitter'),
                  ),
                  */
                  new TextButton(
                    onPressed: () {
          ...
          children: <Widget>[
            ...
            Align(
              ...
              child: ComposeBottomIconWidget(
                ...
                onImageIconSelcted: _onImageIconSelected,
              ),
            ),
          ],
        ),
